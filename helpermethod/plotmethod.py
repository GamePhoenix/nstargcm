...
from classes import Star, Body, System
...
def update(frame, system: System, points, trails, timeArray, time_text):
    for body in system.components:
        startIndex = max(0, frame - 2000 + 1)
        pos = system.positions[body.name][frame]
        trail = np.array(system.positions[body.name][startIndex:frame+1])
...
def animateSystem(system : System, steps : int, frameSkip, limits, simPath : str, timeArray):
    fig = plt.figure()
    ...
    for body in system.components:
        ...
        (trail,) = axis.plot([],[],[], "-", color=color, alpha=0.25)
        points[body] = pt
        trails[body] = trail
    axis.legend()
    anim = FuncAnimation(fig, update, frames=range(0, int(steps), frameSkip), blit=False, interval=30, fargs=(system,points,trails, timeArray, time_text))
    anim.save(f"{simPath}/animation.mp4", writer="ffmpeg", fps=30)
    plt.show()


def paramsOverTime(system : System, timeArray, simPath : str):
    ...
    plt.savefig(f"{simPath}/parameters.png")
    plt.show()